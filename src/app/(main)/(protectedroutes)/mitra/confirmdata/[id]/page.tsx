...
const PartnerConfirmData: React.FC = () => {
  ...
  /**
   * 🔥 FIX PALING AMAN
   * Ambil parameter APAPUN namanya dari URL
   * /mitra/confirmdata/10
   */
  const { id } = useParams();
  const mitraId = id as string;

  const [isLoading, setIsLoading] = useState(true);
  const [isLoadingEdu, setIsLoadingEdu] = useState(true);
  const [eduLevel, setEduLevel] = useState<EduLevel[]>([]);
  ...
  const handleSave = async () => {
    // 🔒 Guard wajib
    if (!mitraId) {
      setErrorMessage("ID Mitra tidak ditemukan di URL");
      return;
    }
    ...
    try {
      ...
      await axios.post(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/savemitra`,
        postData
      );
  ...
  return (
    <div className="p-6 bg-white min-h-screen">
      ...
      <p className="mb-6 text-gray-600">
        Jika ada data yang tidak sesuai, silakan edit terlebih dahulu di halaman
        profil.
      </p>
            ...
            <input
              ...
              value={
                credential.birthdate
                  ? moment(credential.birthdate).format("YYYY-MM-DD")
                  : ""
              }
              onChange={(e) =>
