    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm text-slate-600 font-medium">Memuat detail transaksi...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-lg mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
          ...
          <div className="bg-gradient-to-br from-[#009788] via-[#00796b] to-[#00695c] p-6 text-white">
            ...
            <p className="text-sm opacity-90">Informasi lengkap transaksi Anda</p>
          </div>
          ...
          <div className="p-6 space-y-4">
            ...
            <div className="flex justify-between items-start pb-4 border-b border-slate-200">
              <span className="text-sm font-semibold text-slate-600">TRX ID</span>
              <span className="text-sm text-slate-900 font-mono bg-slate-50 px-2 py-1 rounded">{ppob.trxid}</span>
            </div>

            {/* Merchant Ref */}
            <div className="flex justify-between items-start pb-4 border-b border-slate-200">
              <span className="text-sm font-semibold text-slate-600">Merchant Ref</span>
              <span className="text-sm text-slate-900 font-mono bg-slate-50 px-2 py-1 rounded">{ppob.merchant_ref}</span>
            </div>

            {/* Product */}
            <div className="flex justify-between items-start pb-4 border-b border-slate-200">
              <span className="text-sm font-semibold text-slate-600">Produk</span>
              <span className="text-sm text-slate-900 text-right max-w-[60%]">{callbackProduk}</span>
            </div>

            {/* Product Code */}
            <div className="flex justify-between items-start pb-4 border-b border-slate-200">
              <span className="text-sm font-semibold text-slate-600">Kode Produk</span>
              <span className="text-sm text-slate-900 font-mono">{ppob.code}</span>
            </div>

            {/* Price */}
            <div className="flex justify-between items-start pb-4 border-b border-slate-200">
              <span className="text-sm font-semibold text-slate-600">Harga</span>
            ...
            {ppob.phone && (
              <div className="flex justify-between items-start pb-4 border-b border-slate-200">
                <span className="text-sm font-semibold text-slate-600">Nomor Tujuan</span>
                <span className="text-sm text-slate-900 font-mono">{ppob.phone}</span>
              </div>
            )}
            ...
            {ppob.no_meter_pln && (
              <div className="flex justify-between items-start pb-4 border-b border-slate-200">
                <span className="text-sm font-semibold text-slate-600">No. Meter PLN</span>
                <span className="text-sm text-slate-900 font-mono">{ppob.no_meter_pln}</span>
              </div>
            )}
            ...
            {callbackToken && (
              <div className="pb-4 border-b border-slate-200">
                <span className="text-sm font-semibold text-slate-600 block mb-2">Serial Number / Token</span>
                <div className="bg-[#009788]/5 border border-[#009788]/20 rounded-lg p-3">
                  <span className="text-sm text-slate-900 font-mono font-bold break-all">{callbackToken}</span>
                </div>
              </div>
            )}
            ...
            {callbackNote && (
              <div className="pb-4 border-b border-slate-200">
                <span className="text-sm font-semibold text-slate-600 block mb-2">Catatan Respons</span>
                <p className="text-sm text-slate-700 bg-slate-50 p-3 rounded-lg">{callbackNote}</p>
              </div>
            )}
            ...
            {ppob.voucher_code && (
              <div className="flex justify-between items-start pb-4 border-b border-slate-200">
                ...
                <span className="text-sm text-[#009788] font-semibold bg-[#009788]/10 px-3 py-1 rounded-full">{ppob.voucher_code}</span>
              </div>
            )}

            {/* Created Date */}
            <div className="flex justify-between items-start pb-4 border-b border-slate-200">
              <span className="text-sm font-semibold text-slate-600">Tanggal Buat</span>
              <span className="text-sm text-slate-900">{moment(ppob.created_at).format("DD MMM YYYY HH:mm")}</span>
            </div>

            {/* Payment Status */}
            <div className="flex justify-between items-start pb-4 border-b border-slate-200">
              <span className="text-sm font-semibold text-slate-600">Status Pembayaran</span>
              <span
                className={clsx(
            ...
            {/* Order Status */}
            <div className="flex justify-between items-start pb-4 border-b border-slate-200">
              <span className="text-sm font-semibold text-slate-600">Status Pemesanan</span>
              <span
                className={clsx(
                  "text-xs font-semibold px-3 py-1.5 rounded-full",
            ...
            {payment && (
              <>
                <div className="pt-4">
                  <h3 className="text-lg font-bold text-slate-800 mb-4">Informasi Pembayaran</h3>
                </div>

                <div className="flex justify-between items-start pb-4 border-b border-slate-200">
                  <span className="text-sm font-semibold text-slate-600">TriPay Reference</span>
                  <span className="text-sm text-slate-900 font-mono bg-slate-50 px-2 py-1 rounded">{payment.reference}</span>
                </div>

                <div className="flex justify-between items-start pb-4 border-b border-slate-200">
                  <span className="text-sm font-semibold text-slate-600">Metode Pembayaran</span>
                  <span className="text-sm text-slate-900 font-medium">{payment.payment_method}</span>
                </div>
                ...
                <div className="flex justify-between items-start pb-4">
                  <span className="text-sm font-semibold text-slate-600">Tanggal Pembayaran</span>
                  <span className="text-sm text-slate-900">{moment(payment.created_at).format("DD MMM YYYY HH:mm")}</span>
                </div>
              </>
            )}
          </div>
